...
import { KakaoMapApiService } from 'src/app/services/kakao-map-api.service';
...
export class KakaoMapComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.dataStatusSubscription =
      this.kakaoMapApiService.dataStatusSubject.subscribe((newData) => {
  ...
  displayPlaces(places: any[]) {
    ...
    places.forEach((place, idx) => {
      // 인포 윈도우 생성
      let iwContent = `<div style="padding:5px;">(${idx + 1}) ${
        place.place_name
